...
The more dangerous of the enemies, the bird chases Jack around the level.
...
Bird.prototype.update = function (du) {
    ...
    // Chase after Jack if Bird can see him, first check if Jack still exists.
    ...
    // Reverse direction if colliding with platform.
    ...
    // "Bounce" of walls, floor and ceiling.
    if (this.cx > g_canvas.width || this.cx < 0) {
        this.velX *= -1;
    }
    if (this.cy > levelManager.mapHeight || this.cy < 0) {
        this.velY *= -1;
    }

    // Make sprite look in the right direction.
    if (!this.movingRight){
...
// Check for line of sight to Jack.
